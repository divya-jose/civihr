    ...
    foreach (array('blog','casedashboard') as $name) {
      ...
      if (!empty($dashboardContactId['id']) && $name == 'casedashboard') {
        ...
        if (!CRM_Case_BAO_Case::accessCiviCase()) {
          _hrreport_createDashlet($id, $contact_id, $url, '0', 1);
        }
        elseif($dashboardContactId['values'][$id]['is_active'] == 0) {
          _hrreport_createDashlet($id, $contact_id, $url, '1', 1);
        }
      }
    }
    $i = 1;
    foreach ($report_id as $key=>$val) {
      ...
      if (!empty($dashlet['count']) && $dashlet['count'] > 0) {
        ...
        if (!empty($dashboardContact['id'])) {
          ...
          if (!CRM_Core_Permission::check($dashlet['values'][$dashId]['permission'])) {
            _hrreport_createDashlet($id, $contact_id, $contentUrl, '0', $i);
          }
          elseif($dashboardContact['values'][$id]['is_active'] == 0) {
            _hrreport_createDashlet($id, $contact_id, $contentUrl, '1', $i);
          }
        }
        $i = 0;
      }
...
function _hrreport_createDashlet($dashboardContact, $contact_id, $contentUrl, $setactive,$i) {
  civicrm_api3('DashboardContact', 'create', array("id" => $dashboardContact,'is_active' => $setactive,'contact_id' => $contact_id,'column_no' => $i ,'content' =>  $contentUrl));
}
